...
function images_adminapi_process_image($args)
{
    ...
    if (empty($image) || empty($params)) {
        if ($saveas == 3) {
            ...
            throw new BadParameterException('image or params');
        }
    }
        ...
        if (empty($data)) {
            if ($saveas == 3) {
                ...
                return true;
                 ...
                 throw new BadParameterException('image');
            }
        }
...
// or $phpThumb->setSourceImageResource($gd_image_resource);
    ...
    // Save it to file
    if (empty($save)) {
         throw new EmptyParameterException('save');
    }
...
{
    //{ML_dont_parse 'modules/images/xarclass/phpthumb.class.php'}
